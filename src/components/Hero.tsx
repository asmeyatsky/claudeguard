export default function Hero() {
  const phases = [
    { name: 'ASSESS', color: 'bg-electric' },
    { name: 'CONFIGURE', color: 'bg-amber-accent' },
    { name: 'DEPLOY', color: 'bg-emerald-accent' },
    { name: 'MONITOR', color: 'bg-danger' },
  ]
        ...
        <div className="flex items-center justify-center gap-2 sm:gap-4 mb-12 flex-wrap">
          {phases.map((phase, i) => (
            <div key={phase.name} className="flex items-center gap-2 sm:gap-4">
              <div className={`px-4 sm:px-6 py-2.5 ${phase.color} rounded-lg text-white font-bold text-sm sm:text-base tracking-wide shadow-lg`}>
                {phase.name}
              </div>
              {i < phases.length - 1 && (
