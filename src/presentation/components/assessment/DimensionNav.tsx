...
export default memo(function DimensionNav({ activeDimension, answers, onSelect, showResults, onViewResults }: DimensionNavProps) {
  ...
  return (
    <nav aria-label="Assessment dimensions">
      <div className="space-y-1.5" role="tablist" aria-orientation="vertical" aria-label="Assessment dimensions">
        {DIMENSION_ORDER.map((dimId) => {
          ...
          return (
            <button
              ...
              aria-label={`${dim.name}: ${answered} of ${total} answered${isComplete ? ', complete' : ''}`}
            ...
            >
              <span className="text-lg" aria-hidden="true">{dim.icon}</span>
              <div className="flex-1 min-w-0">
                <div className="font-medium truncate">{dim.shortName}</div>
                <div className="text-[10px] text-navy-500">{answered}/{total}</div>
              </div>
              {isComplete && (
                <svg className="w-4 h-4 text-emerald-accent shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
